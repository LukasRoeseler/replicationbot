...
# load_replied_log()/save_replied_log(), format_reproduction_outcome(),
# format_replication_outcome(), EVALUATION_END_DATE (also used by bot.R / retraction_bot.R)
source("helpers.R")
...
main <- function() {
  ...
  # posts) or as a one-off wider backfill (e.g. 4 weeks / 1000 posts) without
  # duplicating this logic. The default 20-minute window gives a cushion
  # against a delayed/skipped cron tick without risking duplicate replies
  # (dedup is keyed on post_uri via replied_posts.csv, so overlap is
  # harmless either way).
  # ---------------------------------------------------------
  lookback_minutes <- as.numeric(Sys.getenv("LOOKBACK_MINUTES", "20"))
